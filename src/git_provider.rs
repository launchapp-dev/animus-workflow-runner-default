//! Vendored git provider (trait + builtin `git`/`gh` CLI impl).
//!
//! Lifted byte-for-byte from ao-cli
//! `crates/orchestrator-core/src/subject_adapter/git.rs` at rev `ff2d9f25`,
//! where it was deleted (the runner is its only consumer — it executes
//! `post_success.merge`). Vendoring keeps the next ao-cli repin a
//! one-line change.
