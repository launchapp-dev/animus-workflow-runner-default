...
pub async fn execute_workflow_with_hub(
    ...
    if workflow.status == WorkflowStatus::Completed {
        ...
        //
        // TODO(codex-p2): cleanup may report `skipped` when the worktree is
        // dirty (the runtime no longer auto-commits — that is a workflow
        // command phase). We intentionally do NOT fail the workflow in that
        // case: per the thin-runtime decision the final status depends only on
        // the workflow's own phases, not on any git/worktree side-effect.
        // Whether a commit phase ran is the workflow author's composition
        // concern; the skip is surfaced under `post_success.cleanup_worktree`
        // for observability and the worktree (and its work) is preserved.
...
async fn cleanup_worktree_with_fallback(project_root: &str, task: &OrchestratorTask) -> Value {
    ...
    match crate::phase_git::git_has_pending_changes(worktree_path) {
        Ok(true) => {
            return serde_json::json!({
                "status": "skipped",
...
mod requirement_workflow_tests {
    ...
    use super::{execute_workflow_with_hub, workflow_exit_success, WorkflowExecuteParams};
    use orchestrator_core::services::ServiceHub;
    use orchestrator_core::{
        load_agent_runtime_config, write_agent_runtime_config, FileServiceHub, PhaseExecutionMode,
        PhaseManualDefinition, Priority, TaskCreateInput, TaskStatus, TaskType, WorkflowRunInput, WorkflowStatus,
    };
...
mod plugin_pack_fixture_tests {
    ...
    use super::{execute_workflow_with_hub, WorkflowExecuteParams};
    use orchestrator_config::{
...
"#
    ...
    async fn execute_workflow_runs_node_pack_fixture_and_namespaces_pack_mcp() {
        ...
        let result = execute_workflow_with_hub(
            ...
            std::sync::Arc::new(orchestrator_core::FileServiceHub::new(project.path()).expect("file service hub"))
                as std::sync::Arc<dyn ServiceHub>,
            None,
        )
        .await
